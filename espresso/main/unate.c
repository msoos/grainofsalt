...
pset_family A;			/* disposes of A */
{
	 ...
	 */
	restrict_i = set_new(A->sf_size);
	min_set_ord = A->sf_size + 1;
	foreachi_set(A, i, p) {
	    if (SIZE(p) < min_set_ord) {
		set_copy(restrict_i, p);
		min_set_ord = SIZE(p);
	    } else if (SIZE(p) == min_set_ord) {
		set_or(restrict_i, restrict_i, p);
	    }
	}
	...
	/* Check for "essential" columns */
	    ...
	    Abar = unate_complement(abs_covered_many(A, restrict_i));
	    sf_free(A);
	    foreachi_set(Abar, i, p) {
		set_or(p, p, restrict_i);
	    }

	/* else, recur as usual */
	} else {
	    max_i = abs_select_restricted(A, restrict_i);
 ...
 *  also belongs to the set "restrict_i"; weight each column of a set as
 *  1 / (set_ord(p) - 1).
 */
static int
abs_select_restricted(A, restrict_i)
pset_family A;
pset restrict_i;
{
    ...
    count = sf_count_restricted(A, restrict_i);
